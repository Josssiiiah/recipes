    ...
    recipe: {
      ...
      properties: {
        ...
        instructions: {
          type: "string",
          description:
            "Numbered cooking steps as a multiline string. Each line must start with a step number and period, such as '1. Boil the pasta.'",
        },
        ingredients: {
          type: "array",
...
export async function generateRecipeFromImage(
  ...
  if (Buffer.byteLength(imageBase64, "base64") > maxImageBytes) {
    throw new Error("Recipe image is too large. Choose an image under 10 MB.");
  }
...
async function requestStructuredRecipe(
  ...
  const response = await fetcher(openRouterUrl, {
    method: "POST",
    headers,
    body: JSON.stringify({
      model,
...
const recipeSystemPrompt = [
  ...
  "Put each step on its own line, starting with the step number and a period (for example: '1. Boil water.' then a newline, then '2. Drain pasta.').",
